...
The honeypot simulates **75+ real attack surfaces** that are actively scanned on the public internet every day. Each trap responds with convincing fake data — the same response format a real vulnerable server would return — to keep scanners engaged and wasting time.
...
| SharePoint, ColdFusion, Langflow & co. | [Other Services](other-services) | `sharepoint-toolpane`, `coldfusion-admin`, `langflow-apikey`, `apache-solr`, `jenkins-script`, `h2-console`, `grafana`, ... |
